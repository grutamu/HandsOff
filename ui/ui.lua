function ConstructUI()
	if configFunctions == nil then
		...
		function ResizeUI(delta)
			...
			HandsOff_data.HandsOffUI.optionsFrame.scale = scale
		...
		function ClearConfig()
			if HandsOff_data.options[GetSpecialization()] == nil then
				HandsOff_data.options[GetSpecialization()] = {}
			end
			tempTable = {}
			HandsOff_data.options[GetSpecialization()].profile = {}
		end
		...
		-- wtf will be in HandsOff_data.options[GetSpecialization()]
		...
		function CreateNewBox(value,textString,minValue,maxValue,step,base,tip1)
			...
			if HandsOff_data.options[GetSpecialization()][textString.."Status"] == nil then
				HandsOff_data.options[GetSpecialization()][textString.."Status"] = base
			else
				base = HandsOff_data.options[GetSpecialization()][textString.."Status"]
			end
		...
		function CreateNewDrop(value,textString,base,tip1,value1,value2,value3,value4,value5,value6,value7,value8,value9,value10)
			...
			if HandsOff_data.options[GetSpecialization()][textString.."Drop"] == nil then
				HandsOff_data.options[GetSpecialization()][textString.."Drop"] = base
			else
				base = HandsOff_data.options[GetSpecialization()][textString.."Drop"]
			end
			tempTable.base = base
		end
		-- checkbox
		function CreateNewCheck(value,textString,tip1,base)
			...
			if HandsOff_data.options[GetSpecialization()][textString.."Check"] == nil then
				HandsOff_data.options[GetSpecialization()][textString.."Check"] = base
			else
				base = HandsOff_data.options[GetSpecialization()][textString.."Check"]
			end
		...
		function CreateNewTitle(value,textString)
			...
			HandsOff_data.options[GetSpecialization()].profile[#HandsOff_data.options[GetSpecialization()].profile+1] = tempTable
			tempTable = {}
		end
		...
		function CreateNewWrap(value,textString)
			...
			HandsOff_data.options[GetSpecialization()].profile[#HandsOff_data.options[GetSpecialization()].profile+1] = tempTable
			if HandsOff_data.options[GetSpecialization()][textString.."Wrapper"] == nil then
				HandsOff_data.options[GetSpecialization()][textString.."Wrapper"] = true
			end
			tempTable = {}
		end
		-- end of profile
		...
		function WrapsManager()
			HandsOff_data.options[GetSpecialization()].profile.name = HandsOff_data.options[GetSpecialization()].profile[1].name
			createProfile()
			replaceWraps(HandsOff_data.options[GetSpecialization()].profile.name)
			refreshOptions()
		end
		function createProfile()
			--if currentProfile ~= nil and currentProfile ~= HandsOff_data.options[GetSpecialization()].profile then
			--	_G[currentProfileName.."Frame"]:Hide()
			--end
			--currentProfile = HandsOff_data.options[GetSpecialization()].profile
			--currentProfileName = currentProfile[1].name
			--if currentProfile ~= nil and _G[currentProfileName.."Frame"] and HandsOff_data.options[GetSpecialization()].profile[1].name ~= HandsOff_data.options[GetSpecialization()].profile then
            --    _G[currentProfileName.."Frame"]:Hide()--_G[currentProfileName.."Frame"]:Show()
			--end
			--frameCreation(currentProfileName,270,400)
			--local scale = HandsOff_data.HandsOffUI.optionsFrame.scale
		...
		local colorGold = "|cffFFDD11"
		HandsOff_data.HandsOffUI.optionsFrame.options = {
			...
			["General"] = {
				...
				[9] = {
					checkbase = true,
					check = true,
					name = "Start/Stop HandsOff",
					tip = "Uncheck to prevent HandsOff pulsing."
				},
			},
			["Enemies Engine"] = {
				[1] = {
		...
		function radioGeneral(parent,value)
			...
			for i = 1, 4 do
				...
				if thisButton == value then
					_G["options"..thisButton.."Button"].texture:SetTexture(175/255,175/255,175/255,HandsOff_data.HandsOffUI.optionsFrame.color.a)
				else
					_G["options"..thisButton.."Button"].texture:SetTexture(HandsOff_data.HandsOffUI.optionsFrame.color.r/255,HandsOff_data.HandsOffUI.optionsFrame.color.g/255,HandsOff_data.HandsOffUI.optionsFrame.color.b/255,HandsOff_data.HandsOffUI.optionsFrame.color.a)
				end
			end
		end
		-- create frames
		...
		function createOptions(value)
			...
			for i = 1, 12 do
				ypos = ypos + 1
				if HandsOff_data.HandsOffUI.optionsFrame.options[value]
					and HandsOff_data.HandsOffUI.optionsFrame.options[value][i] ~= nil then
					local thisOption,xpos = HandsOff_data.HandsOffUI.optionsFrame.options[value][i],math.floor((i-1)/4)*260
		...
		function refreshOptions()
			...
			for i = 1, 4 do
				...
				if HandsOff_data.HandsOffUI.optionsFrame.options[thisButton] ~= nil then
					-- iterate corresponding frames
					for j = 1, 12 do
						-- current option selected
						local thisOption = HandsOff_data.HandsOffUI.optionsFrame.options[thisButton][j]
						if thisOption ~= nil then
							-- name of the option
							local name = thisOption.name
							-- actual profile options
							local profileOptions = HandsOff_data.options[GetSpecialization()]
		...
		function optionsShow(button)
			...
			for i = 1, 4 do
				if button ~= generalRadios[i] then
					local thisOptionPanel = HandsOff_data.HandsOffUI.optionsFrame.options[generalRadios[i]]
					if thisOptionPanel ~= nil then
						for j = 1, 12 do
							local thisOption = HandsOff_data.HandsOffUI.optionsFrame.options[generalRadios[i]][j]
							if thisOption ~= nil then
								local value = HandsOff_data.HandsOffUI.optionsFrame.options[generalRadios[i]][j].name
	...
	if HandsOff_data.options.selected == nil then
		HandsOff_data.options.selected = "General"
	end
	_G["options"..HandsOff_data.options.selected.."Button"]:Click()
    optionsFrame:Hide()
	if HandsOff_data.options[GetSpecialization()] and HandsOff_data.options[GetSpecialization()]["optionsFrame"] ~= true then
		optionsFrame:Hide()
	end
