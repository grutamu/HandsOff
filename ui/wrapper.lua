...
function createWrapper(parent,value,x,y,wrapperRank)
	if _G[parent..value.."Wrapper"] == nil then
		...
		_G[parent..value.."Wrapper"]:SetAlpha(HandsOff_data.HandsOffUI.alpha)
		-- texture part
		...
		if HandsOff_data.options[GetSpecialization()][value.."Wrapper"] == true then
			_G[parent..value.."Wrapper"].texture:SetTexture(100/255,100/255,100/255,HandsOff_data.HandsOffUI.optionsFrame.color.a)
		else
			_G[parent..value.."Wrapper"].texture:SetTexture(60/255,60/255,60/255,HandsOff_data.HandsOffUI.optionsFrame.color.a)
		end
		...
		_G[parent..value.."Wrapper"]:SetScript("OnClick",function(self)
			...
			if HandsOff_data.options[GetSpecialization()][value.."Wrapper"] ~= true then
				HandsOff_data.options[GetSpecialization()][value.."Wrapper"] = true
				replaceWraps(parent,value)
			else
				HandsOff_data.options[GetSpecialization()][value.."Wrapper"] = false
				replaceWraps(parent,value)
			end
		end)
		-- hover event
		_G[parent..value.."Wrapper"]:SetScript("OnEnter",function(self)
			_G[parent..value.."Wrapper"].texture:SetTexture(200/255,200/255,200/255,HandsOff_data.HandsOffUI.optionsFrame.color.a)
		end)
		-- leave event
		...
		_G[parent..value.."Text"]:SetFont(HandsOff_data.HandsOffUI.font,HandsOff_data.HandsOffUI.fontsize,"THICKOUTLINE")
		_G[parent..value.."Text"]:SetText(value, nil, nil, nil, nil, false)
	end
end
function hideRow(parent,i)
	local value = HandsOff_data.options[GetSpecialization()].profile[i].name
...
function replaceWraps(parent)
	...
	for i = 1, #currentProfile do
		...
		if HandsOff_data.options[GetSpecialization()][name.."Wrapper"] == true then
			isShown = true
		elseif HandsOff_data.options[GetSpecialization()][name.."Wrapper"] == false then
			isShown = false
		end
	end
	local scale = HandsOff_data.HandsOffUI.optionsFrame.scale or 1
